...
class GenerateATSPage extends StatefulWidget {
  ...
  const GenerateATSPage({super.key, required this.onCreditUsed});

  @override
  State<GenerateATSPage> createState() => _GenerateATSPageState();
}

class _GenerateATSPageState extends State<GenerateATSPage> {
  String? fileName;
  bool loading = false;
  int? atsScore;
  ...
  Future<void> generateATS() async {
    ...
    await Future.delayed(const Duration(seconds: 3)); // BUFFERING

    setState(() {
      atsScore = 95 + Random().nextInt(5); // 95–99
      loading = false;
    });
      ...
      children: [
        ...
        Positioned(
          ...
          child: GestureDetector(
            onTap: fileName == null || loading ? null : generateATS,
            child: Container(
              height: 56,
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const Center(
            child: Text("Harsha", style: TextStyle(fontSize: 18))),
